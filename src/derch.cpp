...
int derchflag=0;
MY_DOUBLE_TYPE derch_stepsize=0.0;

static ofstream * pofs=0;

void derch(BOR_CONST MY_DOUBLE_TYPE& _f, BOR_CONST independent_variables & _x,BOR_CONST dvector& _gg,
       int n, BOR_CONST int & _ireturn)
{
  ...
  static MY_DOUBLE_TYPE fsave;
  static int order_flag;
  static MY_DOUBLE_TYPE s, f1, f2, g2, xsave;
  static long int j = 1;
  static int si;
  si=gg.indexmax();
  ...
  while (j > 0)
  {
    ...
    //cout.flush();
    //cin.clear();
    cout << "before read !cout " << !cout << endl;
    cout << "before read !cin " << !cin << endl;
    //int kk=0;
    cin >> j;
    cout << "after read !cin " << !cin << endl;
    //cout << " kk = " << kk << endl;
    if (j == -1)
    {
      ireturn = -1;
...
//    cout << "\n       X           Function     Analytical     Finite Diff;  Index\n";
    ...
    label5:
      ...
      g2=(f1-f2)/(2.*s);
      derchflag=0;
      MY_DOUBLE_TYPE perr= fabs(g2-g(i))/(fabs(g(i))+1.e-6);
   ...
   cout << setw(12) << setscientific() << setprecision(5) << x(i)
        << setw(12) << setscientific() << setprecision(5) << f 
        << setw(12) << setscientific() << setprecision(5) << g(i) 
        << setw(12) << setscientific() << setprecision(5) << g2 
        << setw(5)  << i << endl; 
#else
...
void mytestcin(void)
{
 /*
  int k=0;
  cout << "enter k " << endl;
  cout << "1 before read  !cin = " << !cin << endl;
  cout << "2 before read  !cin = " << !cin << endl;
  //cin >> k;
  cout << "after read  !cin = " << !cin << endl;
 */
  if (!cin)
