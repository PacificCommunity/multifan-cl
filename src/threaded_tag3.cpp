...
extern mf_pvm_manager * mf_pvm;
  ...
  xxxtags::xxxtags(void) : pmsd(0) {}
 
MY_DOUBLE_TYPE mmaxZ[5];
...
int xxxtags::get_variables_from_master_1A(int nt)
{
  ...
  MY_DOUBLE_TYPE tm=adt.get_elapsed_time_and_reset();
...
//    save_identifier_string("RT");
  ...
  gradient_structure::GRAD_STACK1->
   set_gradient_stack(dfadmy_sleep);
}
...
#if !defined(NO_MY_DOUBLE_TYPE)
                        f_by_r(ir)+=sum(gammln(obsgroupedcatch1+1.0L));
#else
...
dvariable xxxtags::tag_catch_equations_calc_loop_2(void)
{
  ...
  do
  {
    ...
    dvariable tmp=fit_pooled_tag_returns();
    MY_DOUBLE_TYPE tm=adt1.get_elapsed_time_and_reset();
    cout << "fit_pooled_tag_returns in loop 2 "  << tmp << endl;
    int nt=0;
    ad_comm::pthread_manager->write_lock_buffer(nt);
    ad_comm::pthread_manager->send_dvariable(tmp,nt); 
    ad_comm::pthread_manager->write_unlock_buffer(nt);
...
void xxxtags::get_constant_data(int tn)
{
  ...
  initial_tag_release_by_length=ad_comm::pthread_manager->get_dmatrix(tn);
...
void dvar_len_fish_stock_history::send_constant_data(int sno)
{
  ...
  ad_comm::pthread_manager->send_int(static_cast<int>(tag_filen),sno);
  ad_comm::pthread_manager->send_int(tag_nlint,sno);
  ad_comm::pthread_manager->send_dmatrix(initial_tag_release_by_length,sno);
