  ...
  d3_array& total_freq,int print_switch)
{
  ...
  {
    fsh.ppstf->wghtcontrib.initialize();
    fsh.ppstf->wghtcontrib_by_realization.initialize();
  }
  ...
  */
  ...
  dvar_vector fp24=fsh.fish_pars(24);
  dvar_vector fp25=fsh.fish_pars(25);

  int blocksize=5000;
  dvariable wght_tmp_dm=0.;
  ...
  if (print_switch)
  {
    ...
    ppofs1 = new ofstream("wght_dmsizemult");
    int maxgp=max(fsh.wtcs->wght_rho_group_ptr);
    (*ppofs1) << " average sample sizes " <<endl;
    for (int i=1;i<=maxgp;i++)
    {
      (*ppofs1) << "  " << fsh.wtcs->average_sample_size(i);
    }
    (*ppofs1) << endl;
  ...
  for (int ii=1;ii<=fsh.wtcs->tot_nfi;ii+=blocksize)
  {
    ...
    {
      if (ic>fsh.wtcs->tot_nfi) break;
      int sz=sizes(ic);
      if (sz<fsh.wtcs->minsize)
      {
      ...
      if (sz>1)
      {
        int ir=fsh.wtcs->reg(ic);
        int ip=fsh.wtcs->per(ic);
        int fi=fsh.wtcs->finc(ic);
        int pp=fsh.parent(ir,ip,fi);
        int gp=fsh.wtcs->wght_rho_group_ptr(pp);  //group this fishery is in   
        int lbic=fsh.wtcs->left_bound(ic);
        int rbic=fsh.wtcs->right_bound(ic);
        dvector lf=fsh.wght_freq(ir,ip,fi);
        dvector OP(lbic,rbic);
        OP=lf(lbic,rbic);
        dvar_vector P(lbic,rbic);
        dvar_vector plf=fsh.wtprob(ir,ip,fi);
        // XXXXXXXXXXXXXXXXX
        P=plf(lbic,rbic);
        OP.shift(1);
        P.shift(1);
        ...
        MY_DOUBLE_TYPE wss=fsh.wght_sample_size(ir,ip,fi);
        ...
        MY_DOUBLE_TYPE relative_sample_size=min_sample_size+
          wss/(min_sample_size+fsh.wtcs->average_sample_size(gp));
        
        dvariable size_multiplier_dm=
          pow(relative_sample_size,fp25(pp));

        dvariable dmlambda=exp(fp24(pp))*size_multiplier_dm;
        ...
        if (ppofs1)
        {
          ...
          MY_DOUBLE_TYPE moment_estimator=m/sum(elem_div(square(value(OP-P)),
            value(1.e-6+P)));
          (*ppofs1) << "   " << fsh.wtcs->wght_rho_group_ptr(pp) 
        ...
        if (pofs1)
        {
          ...
          tmp1.initialize();
          tmp1(fsh.wtcs->left_bound(ic),fsh.wtcs->right_bound(ic)).shift(1)=OP;
//          (*pofs1) << ir << " " << ip << " " << fi << endl;
          (*pofs1) << ir << " " << ip << " " << fi << " " << pp << endl;
          (*pofs1) << tmp1 << endl;
          tmp1(fsh.wtcs->left_bound(ic),fsh.wtcs->right_bound(ic)).shift(1)=value(P); //NMD_31May2016
        ...
        if (print_switch && !fsh.af170q0)
        {
          ...
          fsh.ppstf->wghtcontrib(fsh.parent(ir,ip,fi)) -= value(ttmp0);
          fsh.ppstf->wghtcontrib(fsh.parent(ir,ip,fi)) -= value(ttmp);
//          fsh.ppstf->wghtcontrib_by_realization(ir,ip,fi) = value(ttmp0);
          fsh.ppstf->wghtcontrib_by_realization(ir,ip,fi) -= value(ttmp0);  //NMD_25apr2022
          fsh.ppstf->wghtcontrib_by_realization(ir,ip,fi) -= value(ttmp);
        }
      }
    }
  }
  ...
  d3_array& total_freq,int print_switch)
{
  ...
  if(allocated(fsh.wtcs->wght_rho_group_ptr))
  {
    gpvector=fsh.wtcs->wght_rho_group_ptr;
  }
  else if(sum(column(fsh.fish_flags,77))>0)
  {	  
    gpvector=column(fsh.fish_flags,77);
  }  
  else
  {
...
// NMD_30May2017 - ensure average_sample_size calculation
  ...
  ivector num_samples;
  if(allocated(fsh.wtcs->average_sample_size))
  {
    average_sample_size=fsh.wtcs->average_sample_size;
  }
  else
  {
    ...
    for (int ip=1;ip<=fsh.num_fish_periods(ir);ip++)
    {
      for (int fi=1;fi<=fsh.num_fish_incidents(ir,ip);fi++)
      {
        if(fsh.wght_sample_size(ir,ip,fi)>0)
        {
          ...
          dvar_vector P=fsh.wtprob(ir,ip,fi)+.0001;;
	  P/=sum(P);
          MY_DOUBLE_TYPE wss=fsh.wght_sample_size(ir,ip,fi);
          //  add a small amount to relative sample size in case it is almost 0
          // which screws up the differentiabity
...
//             wss/(min_sample_size+fsh.wtcs->average_sample_size(gp));
          MY_DOUBLE_TYPE relative_sample_size=min_sample_size+
             wss/(min_sample_size+average_sample_size(gp));
