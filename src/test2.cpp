...
void dvar_len_fish_stock_history::
  do_newton_raphson(int ir,int ip,dvariable& ffpen)
{
  ...
  do_newton_raphson_with_totcatch(int ir,int ip,dvariable& ffpen)
{
  ...
  dvar_vector kc=get_kludged_catch_with_totcatch(ir,ip,ffpen,enf);

  dvar_vector qq=elem_div(kc,sel*enf);
  qq/=4.0;
  dvar_vector TC(1,nfi);
  dvar_vector diff(1,nfi);
...
/*
dvar4_array dvar_len_fish_stock_history::mean_weight_calcs(void)
{
  pmsd_error();
  ...
  for (int ir=1;ir<=num_regions;ir++)
  {
    for (int ip=1;ip<=num_fish_periods(ir);ip++)  
    {
      ...
      {                                         // incidents for this period
        mean_weight(ir,ip,fi)=get_mean_weight_at_age(ir,ip,fi);
      }
    }
  }
}
...
dvar_vector dvar_len_fish_stock_history::get_mean_weight_at_age
  (int ir,int ip,int fi)
{
  dvar_vector mw(1,nage);
  dvariable sv27=get_sv_region(ir,27); 
  dvariable sv28=get_sv_region(ir,28); 
  if (value(sv28)==3.0) 
  {
    cerr << "Never tested" << endl;
    ad_exit(1);
    mw=len_wt_coff* 
       (pow(mean_length(ir,ip,fi),3)+
       3.0*elem_prod(mean_length(ir,ip,fi),vars(ir,ip,fi)));
  ...
  else
  {
    for (int j=1;j<=nage;j++)
    {  
      mw(j)=normal_length_to_weight(0.5,-3.5,
        mean_length(ir,ip,fi,j),
          sqrt(vars(ir,ip,fi,j)),value(sv27), 
        value(sv28));  
    }
  }
  return mw;
