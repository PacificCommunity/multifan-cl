...
//        }
        ...
        len_tmp-=vN*(obsc*log(predc));
        ftmp-=vN*(obsc*log(predc)); //NMD_8dec2023

        if (fsh.ppstf && print_switch && !fsh.af170q0)
        {
          fsh.ppstf->lencontrib(pp) += value(ftmp);
          fsh.ppstf->lencontrib_by_realization(ir,ip,fi) = value(ftmp);
        }
...
//          (*pofs1) << ir << " " << ip << " " << fi << endl;
          ...
          (*pofs1) << tmp1 << endl;
          tmp1(left_bound(ic),right_bound(ic)).shift(1)=value(predc); //NMD_31May2016
